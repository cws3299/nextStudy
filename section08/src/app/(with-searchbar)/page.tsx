...
export default async function Home() {
  return (
    <div className={style.container}>
      <section>
        <h3>지금 추천하는 도서</h3>
        <Suspense fallback={<BookItemListSkeleton count={3} />}>
          <RecoBooks />
        </Suspense>
      </section>
      <section>
        <h3>등록된 모든 도서</h3>
        <Suspense fallback={<BookItemListSkeleton count={10} />}>
          <AllBooks />
        </Suspense>
      </section>
    </div>
...
// Prallel Route 병렬 라우트
// 하나의 화면 안에 다른 여러 라우트(페이지)를 표현함 page.tsx
// 하나의 화면에 여러 page.tsx 컴포넌트를 표현함 -> sns, 관리자 서비스 등에서 굉장히 도움됨
